...
type MedicalMetadata struct {
	...
	PendingRequests []string `json:"pendingRequests"` // List of HospitalIDs requesting access
}
...
// Helper: retrieve metadata from state (private, no access control)
func (c *MedicalContract) _getMetadataInternal(ctx contractapi.TransactionContextInterface, recordId string) (*MedicalMetadata, error) {
...
// Enforces Access Control: requester must be in AccessList or be the owner.
func (c *MedicalContract) ReadMetadata(ctx contractapi.TransactionContextInterface, recordId string, requesterId string) (*MedicalMetadata, error) {
	metadata, err := c._getMetadataInternal(ctx, recordId)
	if err != nil {
		return nil, err
	}
	...
	hasAccess := false
	for _, id := range metadata.AccessList {
		if id == requesterId {
			hasAccess = true
			break
		}
	...
	if !hasAccess {
		return nil, fmt.Errorf("access denied for requester: %s", requesterId)
	}
...
// RequestAccess allows a hospital to request access to a record
func (c *MedicalContract) RequestAccess(ctx contractapi.TransactionContextInterface, recordId string, requesterId string) error {
	metadata, err := c._getMetadataInternal(ctx, recordId)
	if err != nil {
		return err
	}
	...
	for _, id := range metadata.AccessList {
		if id == requesterId {
			return fmt.Errorf("requester %s already has access", requesterId)
		}
	}

	// Check if already in pending requests
	for _, id := range metadata.PendingRequests {
		if id == requesterId {
			return nil // Already requested
		}
	}

	metadata.PendingRequests = append(metadata.PendingRequests, requesterId)
...
// GrantAccess adds a hospital ID to the access list and removes from pending
func (c *MedicalContract) GrantAccess(ctx contractapi.TransactionContextInterface, recordId string, targetHospitalId string) error {
	...
	newPending := []string{}
	for _, p := range metadata.PendingRequests {
		if p != targetHospitalId {
			newPending = append(newPending, p)
		}
	}
	metadata.PendingRequests = newPending
