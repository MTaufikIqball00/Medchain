    ...
    const processRecords = async () => {
      ...
      for (const block of filteredBlocks) {
        ...
        if (matches) {
          ...
          const txHash = (blockHash && blockHash !== "PENDING" && blockHash !== "GENESIS") ? blockHash : undefined;
          rows.push({
            ...data,
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-100 text-slate-800 print:bg-transparent">
                  <tr>
                    ...
                    <th className="px-3 py-2 border border-slate-300 text-xs font-bold w-40 print:hidden">Blockchain TX</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    currentData.map((row, index) => (
                      <tr key={index} className="hover:bg-blue-50 print:hover:bg-transparent odd:bg-white even:bg-slate-50 print:even:bg-transparent">
                        ...
                        <td className="px-3 py-2 border border-slate-300 text-xs text-slate-600 print:hidden">
                          {(row.ethTxHash || row.fabricTxId) ? (
                            <div className="flex items-center gap-1">
                              <span className="font-mono text-[11px] text-slate-700 truncate max-w-[200px]" title={row.ethTxHash || row.fabricTxId}>
                                {(row.ethTxHash || row.fabricTxId)!.substring(0, 16)}...
                              </span>
                              <button
                                onClick={() => handleViewTransaction(row.ethTxHash || row.fabricTxId!)}
